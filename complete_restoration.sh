...
echo "=== COMPLETE RESTORATION TO WORKING STATE ==="
echo "Timestamp: $(date)"
echo "Reverting ALL changes back to original working configuration..."
echo

cd /var/www/html

# 1. Restore original .env configuration (the REAL working one)
echo "1. Restoring original working .env configuration..."
...
echo "   ✅ Restored original .env with correct database credentials"

# 2. Restore original Kernel.php (working version)
echo "2. Restoring original Kernel.php..."
...
echo "   ✅ Restored original Kernel.php"

# 3. Restore the ORIGINAL working routes (from your workspace)
echo "3. Restoring original working routes..."
...
echo "   ✅ Restored original working routes"

# 4. Test database connection with original credentials
echo "4. Testing original database connection..."
mysql -u ebrew_user -p'secure_db_password_2024' ebrew_laravel_db -e "SELECT COUNT(*) as user_count FROM users;" 2>/dev/null && echo "   ✅ Original database connection works!" || echo "   ⚠️ Original database connection failed"

# 5. Check if admin user exists in ORIGINAL database
echo "5. Checking admin user in original database..."
mysql -u ebrew_user -p'secure_db_password_2024' ebrew_laravel_db -e "SELECT id, name, email, role, is_admin FROM users WHERE email = 'abhishake.a@gmail.com';" 2>/dev/null || echo "   ⚠️ Admin user check in original DB failed"

# 6. Clear ALL caches completely
echo "6. Clearing ALL Laravel caches..."
php artisan config:clear 2>/dev/null || true
php artisan cache:clear 2>/dev/null || true
php artisan route:clear 2>/dev/null || true
php artisan view:clear 2>/dev/null || true

# Remove any compiled files that might be cached
...
# 9. Test database connection via Laravel
echo "9. Testing Laravel database connection..."
cd /var/www/html
php artisan migrate:status 2>/dev/null && echo "   ✅ Laravel DB connection restored!" || echo "   ⚠️ Laravel DB connection still has issues"

# 10. Test routes
echo "10. Testing routes..."
php artisan route:list | head -5 2>/dev/null && echo "   ✅ Routes working!" || echo "   ⚠️ Routes still have issues"
...
echo "✅ Restored original Kernel.php"
echo "✅ Restored original working routes"
echo "✅ Tested original database connection"
echo "✅ Cleared all caches and compiled files"
echo "✅ Set proper permissions"
echo "✅ Restarted Apache"
echo
echo "🎯 ORIGINAL WORKING CONFIGURATION RESTORED:"
...
echo "4. Database test: http://13.60.43.49/debug/database"
echo "5. Admin test: http://13.60.43.49/debug/admin-test"
echo
echo "Website should be working exactly as it was before!"